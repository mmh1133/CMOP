...
abundance <- c(d1m.crypto, d2m.crypto, d3m.crypto, d7m.crypto, d8m.crypto, d9m.crypto, d15m.crypto, d16m.crypto, d17m.crypto)
...
d3m.c.sd <- sd(day3.crypto$abundance, na.rm=T)
d4m.c.sd <- sd(day4.crypto$abundance, na.rm=T)
d5m.c.sd <- sd(day5.crypto$abundance, na.rm=T)  #NA
d6m.c.sd <- sd(day6.crypto$abundance, na.rm=T)  #NA
d7m.c.sd <- sd(day7.crypto$abundance, na.rm=T)
d8m.c.sd <- sd(day8.crypto$abundance, na.rm=T)
d9m.c.sd <- sd(day9.crypto$abundance, na.rm=T)
d10m.c.sd <- sd(day10.crypto$abundance, na.rm=T) #NA
d11m.c.sd <- sd(day11.crypto$abundance, na.rm=T) #NA
d12m.c.sd <- sd(day12.crypto$abundance, na.rm=T) #NA
d13m.c.sd <- sd(day13.crypto$abundance, na.rm=T)
d14m.c.sd <- sd(day14.crypto$abundance, na.rm=T)  
d15m.c.sd <- sd(day15.crypto$abundance, na.rm=T)  
d16m.c.sd <- sd(day16.crypto$abundance, na.rm=T)
d17m.c.sd <- sd(day17.crypto$abundance, na.rm=T)
d18m.c.sd <- sd(day18.crypto$abundance, na.rm=T) #NA
d19m.c.sd <- sd(day19.crypto$abundance, na.rm=T) #NA
d20m.c.sd <- sd(day20.crypto$abundance, na.rm=T) #NA
d21m.c.sd <- sd(day21.crypto$abundance, na.rm=T)
d22m.c.sd <- sd(day22.crypto$abundance, na.rm=T)
d23m.c.sd <- sd(day23.crypto$abundance, na.rm=T) #NA

abundance.sd <- c(d1m.c.sd, d2m.c.sd, d3m.c.sd, d7m.c.sd, d8m.c.sd, d9m.c.sd, d15m.c.sd, d16m.c.sd, d17m.c.sd)

abundance.n <- c(368, 189, 174, 214, 325, 12, 290, 261, 65)

abundance.se <- (abundance.sd/abundance.n)^2
...
yay$daily.GRmean <- rollapply(data=yay$h.dr.mean, width=24, FUN=sum, na.rm=T, fill=NA)*24
...
dr.w3.new <- subset(yay, time > as.POSIXct("2013-09-25 09:00:00",tz='GMT', origin="1970-01-01") & time < as.POSIXct("2013-09-28 24:00:00",tz='GMT', origin="1970-01-01"))
...
d13m <- mean(day13$daily.GRmean, na.rm=T) #para
d14m <- mean(day14$daily.GRmean, na.rm=T) #para
...
d21m <- mean(day21$daily.GRmean, na.rm=T) #para
d22m <- mean(day22$daily.GRmean, na.rm=T) #para
d23m <- mean(day23$daily.GRmean, na.rm=T) #NA

dm <- c(d1m, d2m, d3m, d7m, d8m, d9m, d13m, d14m, d15m, d16m, d17m, d21m, d22m)
dm.para <- c(d1m, d2m, d3m, d7m, d8m, d9m, d15m, d16m, d17m)

#standard deviation
d1m.sd <- sd(day1$daily.GRmean, na.rm=T)
d2m.sd <- sd(day2$daily.GRmean, na.rm=T)
d3m.sd <- sd(day3$daily.GRmean, na.rm=T)
d4m.sd <- sd(day4$daily.GRmean, na.rm=T)
d5m.sd <- sd(day5$daily.GRmean, na.rm=T)  #NA
d6m.sd <- sd(day6$daily.GRmean, na.rm=T)  #NA
d7m.sd <- sd(day7$daily.GRmean, na.rm=T)
d8m.sd <- sd(day8$daily.GRmean, na.rm=T)
d9m.sd <- sd(day9$daily.GRmean, na.rm=T)
d10m.sd <- sd(day10$daily.GRmean, na.rm=T) #NA
d11m.sd <- sd(day11$daily.GRmean, na.rm=T) #NA
d12m.sd <- sd(day12$daily.GRmean, na.rm=T) #NA
d13m.sd <- sd(day13$daily.GRmean, na.rm=T)
d14m.sd <- sd(day14$daily.GRmean, na.rm=T)  
d15m.sd <- sd(day15$daily.GRmean, na.rm=T)  
d16m.sd <- sd(day16$daily.GRmean, na.rm=T)
d17m.sd <- sd(day17$daily.GRmean, na.rm=T)
d18m.sd <- sd(day18$daily.GRmean, na.rm=T) #NA
d19m.sd <- sd(day19$daily.GRmean, na.rm=T) #NA
d20m.sd <- sd(day20$daily.GRmean, na.rm=T) #NA
d21m.sd <- sd(day21$daily.GRmean, na.rm=T)
d22m.sd <- sd(day22$daily.GRmean, na.rm=T)
d23m.sd <- sd(day23$daily.GRmean, na.rm=T) #NA

dm.sd <- c(d1m.sd, d2m.sd, d3m.sd, d7m.sd, d8m.sd, d9m.sd, d15m.sd, d16m.sd, d17m.sd)

dm.n <- c(24, 24, 19, 23, 24, 5, 19, 24, 24, 24, 11, 23, 4)
dm.n2 <- c(24, 24, 19, 23, 24, 5, 24, 24, 11)
...
par.cum <- c(d1m.p, d2m.p, d3m.p, d7m.p, d8m.p, d9m.p, d15m.p, d16m.p, d17m.p)
...
ph <- c(0.9, 1.1, 0.9, 0.9, 1.1, 0.6, 0.7, 0.7, 1.0) #phosphate
a <- c(5.0, 6.5, 5.1, 4.6, 5.3, 13.5, 3.6, 2.5, 8.1) #ammonium
...
production <- dm.para*abundance
production.se <- (abundance.se+dm.se)^(1/2)
tn <- (n+a)
...
# TS #
#par(mar=c(5,12,4,12))
plot(sal$time, sal$water_salinity, lwd=1.5, pch=16, xlab="", ylab="", cex=0.2, las=1, cex.axis=1, xlim=c(as.POSIXct("2013-09-11 00:00:00"), as.POSIXct("2013-10-04 07:00:00")))
...
mtext(2, text="salinity (psu)", line=2.5, cex=1)
par(new=T)
plot(sal$time, sal$water_temperature, lwd=1.5, pch=16, xlab="", ylab="", cex.lab=1,  axes=F, col="darkgrey", cex=0.2, las=1, cex.axis=1, xlim=c(as.POSIXct("2013-09-11 00:00:00"), as.POSIXct("2013-10-04 07:00:00")))
...
axis(4, cex.axis=1)
mtext(4, text="water temperature \n(degree C)", line=4, cex=1)
mtext("A", side=3, cex=1.5, adj=0)
legend("topright", c("salinity", "temperature"), lty=c(1,1), lwd=c(4,4), col=c("black", "darkgrey"), cex=0.9)
...
plot(as.POSIXct(par.smooth1$smooth.x, origin="1970-01-01", tz='GMT'), par.smooth1$smooth.y, lwd=1.5, pch=16, cex=1, xlab="", ylab="", col="black",  cex.axis=1, las=1, type="l", xlim=c(as.POSIXct("2013-09-11 00:00:00"), as.POSIXct("2013-10-04 07:00:00")))
par(new=T)
plot(as.POSIXct(par.smooth2$smooth.x, origin="1970-01-01", tz='GMT'), par.smooth2$smooth.y, lwd=1.5, pch=16, cex=1, xlab="", ylab="", col="black",  cex.axis=1, las=1, type="l", axes=F, xlim=c(as.POSIXct("2013-09-11 00:00:00"), as.POSIXct("2013-10-04 07:00:00")))
mtext(2, text="PAR", line=2.5, cex=1)
mtext("B", side=3, cex=1.5, adj=0)

# nutrients #
#par(mar=c(5,12,4,12))
plot(pre.flu2$time2, pre.flu2$Nitrate, lwd=2, pch=16, cex=1.5, xlab="", ylab="", cex.lab=1, ylim=c(0,22), las=1, cex.axis=1, xlim=c(as.POSIXct("2013-09-11 00:00:00"), as.POSIXct("2013-10-04 07:00:00")))
mtext(substitute(paste("nitrate (", mu,"M)")), side=2, cex=1, line=2)
par(new=T)
plot(pre.flu2$time2, pre.flu2$Ammonium, lwd=2, pch=0, cex=1, xlab="", ylab="", axes=F, ylim=c(0,100), las=1, cex.axis=1, xlim=c(as.POSIXct("2013-09-11 00:00:00"), as.POSIXct("2013-10-04 07:00:00")))
axis(2, line=4.3, cex.axis=1)
mtext(substitute(paste("ammonium (", mu, "M)")), side=2, line=6.3, cex=1)
par(new=T)
plot(pre.flu2$time2, pre.flu2$Phosphate, lwd=2, pch=4, cex=1, xlab="", ylab="", axes=F, ylim=c(0, 2), las=1, cex.axis=1, xlim=c(as.POSIXct("2013-09-11 00:00:00"), as.POSIXct("2013-10-04 07:00:00")))
axis(4, cex.axis=1)
mtext(substitute(paste("phosphate (", mu, "M)")),side=4, line=2.8, cex=1)
legend("topright", c("nitrate", "ammonium", "phosphate"), pch=c(16, 0, 4), cex=1, ncol=3)
...
#week 1 log scale
#par(mai=c(1,1.5,1,1))
plot(crypto.week1$time, crypto.week1$abundance, xaxt="n", xlab="", lwd=1, pch=16, ylab= "", cex.lab=1, log="y", col="darkgrey", las=1, cex.axis=1, ylim=c(0.0005, 40), cex=0.75)
...
axis.POSIXct(1, crypto.week1$time, at=seq(min(crypto.week1$time, na.rm=T), max(crypto.week1$time, na.rm=T), by=60*60*6), format="%m-%d %H:%M", cex.axis=1)
mtext("A", side=3, cex=1.5, line=0, adj=0)
mtext(substitute(paste("abundance 10"^{6}, " cells L"^{-1})), side=2, cex=1.5, outer=T, line=-3)
...
rect(as.POSIXct("2013-09-12 00:55:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-09-12 07:24:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
rect(as.POSIXct("2013-09-12 12:44:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-09-12 18:43:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
...
#week 2 log scale
#par(mai=c(1,1.5,1,1))
plot(crypto.week2$time, crypto.week2$abundance, xaxt="n", xlab="", lwd=1, pch=16, ylab="", cex.lab=1, log="y", col="darkgrey", las=1, cex.axis=1, ylim=c(0.0005, 40), cex=0.75)
...
axis.POSIXct(1, crypto.week2$time, at=seq(as.POSIXct("2013-09-16 20:00:00", origin="1970-01-01", tz='GMT'), max(crypto.week2$time, na.rm=T), by=60*60*6), format="%m-%d %H:%M", cex.axis=1)
mtext("B", side=3, cex=1.5, adj=0)

rect(as.POSIXct("2013-09-16 17:43:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-09-16 23:36:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
rect(as.POSIXct("2013-09-17 06:22:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-09-17 12:32:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
rect(as.POSIXct("2013-09-17 18:38:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-09-18 00:32:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
rect(as.POSIXct("2013-09-18 07:06:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-09-18 13:13:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
rect(as.POSIXct("2013-09-18 19:29:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-09-19 01:23:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
...
#week 3 log scale
#par(mai=c(1,1.5,1,1))
plot(crypto.week3$time, crypto.week3$abundance, xaxt="n", xlab="", lwd=1, pch=16, ylab="", cex.lab=1, ylog=T, log="y", col="darkgrey", las=1, cex.axis=1, ylim=c(0.0005, 40), cex=0.75)
...
axis.POSIXct(1, crypto.week3$time, at=seq(min(crypto.week3$time, na.rm=T), max(crypto.week3$time, na.rm=T), by=60*60*6), format="%m-%d %H:%M", cex.axis=1)
mtext("C", side=3, cex=1.5, adj=0)

rect(as.POSIXct("2013-09-23 23:07:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-09-24 05:22:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
rect(as.POSIXct("2013-09-24 10:57:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-09-24 16:47:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
rect(as.POSIXct("2013-09-24 23:52:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-09-25 06:16:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
rect(as.POSIXct("2013-09-25 11:40:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-09-25 17:28:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
rect(as.POSIXct("2013-09-26 00:43:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-09-26 07:16:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
rect(as.POSIXct("2013-09-26 12:33:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-09-26 18:18:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
rect(as.POSIXct("2013-09-16 17:43:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-09-16 23:36:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
...
#week 4 log scale
#par(mai=c(1,1.5,1,1))
plot(crypto.week4$time, crypto.week4$abundance, xaxt="n", xlab="", lwd=1, pch=16, ylab="", cex.lab=1, ylog=T, log="y", col="darkgrey", las=1, cex.axis=1, ylim=c(0.0005, 40), cex=0.75)
...
axis.POSIXct(1, crypto.week4$time, at=seq(as.POSIXct("2013-09-30 18:50:00", origin="1970-01-01", tz='GMT'), max(crypto.week4$time, na.rm=T), by=60*60*6), format="%m-%d %H:%M", cex.axis=1)
mtext("D", side=3, cex=1.5, adj=0)

rect(as.POSIXct("2013-09-30 17:03:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-09-30 22:53:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
rect(as.POSIXct("2013-10-01 05:32:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-10-01 11:48:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
rect(as.POSIXct("2013-10-01 17:52:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-10-01 22:45:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
rect(as.POSIXct("2013-10-02 06:12:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-10-02 12:23:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
rect(as.POSIXct("2013-10-02 18:37:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-10-03 00:32:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.07), border=NA)
...
png(filename="/Users/mariaham/CMOP/manuscript/fourth_draft_figures/div3_fig4.png", width=1500, height=975, units="px", res=150)
#quartz("Quartz", width=10, height=6.5)
par(mfrow=c(3,1), mar=c(1.15,5,1.5,2)+0.8, pty="m", cex.axis=1)


#week 1
#par(mai=c(1,1.5,1,1))
plotCI(dr.w1$time, dr.w1$daily.GRmean, uiw= dr.w1$daily.GRse, sfrac=0, pch=NA, 	xlab="", ylab="", cex.lab=1, xaxt="n", las=1, ylim=c(0,3.5), xlim=c(as.POSIXct("2013-09-11 00:00:00", origin="1970-01-01", tz='GMT'), as.POSIXct("2013-09-14 00:00:00", origin="1970-01-01", tz='GMT')))
axis.POSIXct(1, dr.w1$time, at=seq(as.POSIXct("2013-09-11 00:00:00", origin="1970-01-01", tz='GMT'), as.POSIXct("2013-09-14 00:00:00", origin="1970-01-01", tz='GMT'), by=60*60*12), format="%m-%d %H:%M", cex.axis=1)
par(new=T)
plot(dr.w1$time, dr.w1$daily.GRmean, xlab="", ylab="", axes=F, type="l", ylim=c(0,3.5), xlim=c(as.POSIXct("2013-09-11 00:00:00", origin="1970-01-01", tz='GMT'), as.POSIXct("2013-09-14 00:00:00", origin="1970-01-01", tz='GMT')))
mtext("A", side=3, cex=1.5, line=0, adj=0)
mtext(text="mean daily division rate", side=2, cex=1.5, outer=T, line=-2)
...
#week2
#par(mai=c(1,1.5,1,1))
plotCI(dr.w2$time, dr.w2$daily.GRmean, uiw= dr.w2$daily.GRse, sfrac=0, pch=NA, 	xlab="", ylab="", cex.lab=1, xaxt="n", las=1, ylim=c(0,3.5),  xlim=c(as.POSIXct("2013-09-17 06:00:00", origin="1970-01-01", tz='GMT'), as.POSIXct("2013-09-19 12:00:00", origin="1970-01-01", tz='GMT')))
axis.POSIXct(1, dr.w2$time, at=seq(as.POSIXct("2013-09-17 07:00:00", origin="1970-01-01", tz='GMT'), max(dr.w2$time, na.rm=T), by=60*60*12), format="%m-%d %H:%M", cex.axis=1)
par(new=T)
plot(dr.w2$time, dr.w2$daily.GRmean, xlab="", ylab="", axes=F, type="l", ylim=c(0,3.5),  xlim=c(as.POSIXct("2013-09-17 06:00:00", origin="1970-01-01", tz='GMT'), as.POSIXct("2013-09-19 12:00:00", origin="1970-01-01", tz='GMT')))
mtext("B", side=3, cex=1.5, line=0, adj=0)
...
#week3
#par(mai=c(1,1.5,1,1))
plotCI(dr.w3.new$time, dr.w3.new$daily.GRmean, uiw= dr.w3.new$daily.GRse, sfrac=0, pch=NA, 	xlab="", ylab="", cex.lab=1, xaxt="n", las=1, ylim=c(0,3.5), xlim=c(as.POSIXct("2013-09-25 09:00:00", origin="1970-01-01", tz='GMT'), as.POSIXct("2013-09-27 20:00:00", origin="1970-01-01", tz='GMT')))
axis.POSIXct(1, dr.w3.new$time, at=seq(as.POSIXct("2013-09-25 09:00:00", origin="1970-01-01", tz='GMT'), max(dr.w3.new$time, na.rm=T), by=60*60*12), format="%m-%d %H:%M", cex.axis=1)
par(new=T)
plot(dr.w3.new$time, dr.w3.new$daily.GRmean, xlab="", ylab="", axes=F, type="l", ylim=c(0,3.5), xlim=c(as.POSIXct("2013-09-25 09:00:00", origin="1970-01-01", tz='GMT'), as.POSIXct("2013-09-27 20:00:00", origin="1970-01-01", tz='GMT')))
mtext("C", side=3, cex=1.5, line=0, adj=0)
...
# #week4
# #par(mai=c(1,1.5,1,1))
# plotCI(dr.w4$time,dr.w4$daily.GRmean, uiw= dr.w4$daily.GRse, sfrac=0, pch=NA, 	xlab="", ylab="", cex.lab=1, xaxt="n", las=1, ylim=c(0,11), xlim=c(as.POSIXct("2013-10-01 06:00:00", origin="1970-01-01", tz='GMT'), as.POSIXct("2013-10-02 13:00:00", origin="1970-01-01", tz='GMT')) )
# axis.POSIXct(1, dr.w4$time, at=seq(as.POSIXct("2013-09-29 11:00:00", origin="1970-01-01", tz='GMT'), max(dr.w4$time, na.rm=T), by=60*60*12), format="%m-%d %H:%M", cex.axis=1)
# par(new=T)
# plot(dr.w4$time, dr.w4$daily.GRmean, xlab="", ylab="", axes=F, type="l", ylim=c(0,11), xlim=c(as.POSIXct("2013-10-01 06:00:00", origin="1970-01-01", tz='GMT'), as.POSIXct("2013-10-02 13:00:00", origin="1970-01-01", tz='GMT')) )
# mtext("D", side=3, cex=1.5, line=0, adj=0)
...
#nitrate
#par(mai=c(1,1,1,7))
plot(cmop$n, cmop$dm, pch=16, xlab="nitrate", ylab="mean daily division rate", cex.lab=1.7, cex=1.5, xlim=c(0,14), cex.axis=1.7)
#res1=lm(cmop$dm~cmop$n)
res1 <- lmodel2(cmop$dm~cmop$n, data=cmop, nperm=0, range.x="relative", range.y="relative")
par(new=T)
plot(res1, "MA", ylab="", xlab="", main="", col="black", xlim=c(0,14), cex.axis=1.7)
text(5.5,8, substitute(paste("R"^{2}, "=0.017")), cex=2)
mtext("A", side=3, cex=2, adj=0)

#phosphate
# par(mai=c(1,1,1,7))
plot(cmop$ph, cmop$dm, pch=16, xlab="phosphate", ylab="", cex.lab=1.7, cex=1.5, xlim=c(0,1.3), cex.axis=1.7)
#res2=lm(cmop$dm~cmop$ph)
res2 <- lmodel2(cmop$dm~cmop$ph, data=cmop, nperm=0, range.x="relative", range.y="relative")
par(new=T)
plot(res2, "MA", ylab="", xlab="", main="", col="black", xlim=c(0,1.3), cex.axis=1.7)
text(0.95,8, substitute(paste("R"^{2}, "=0.267")), cex=2)
mtext("B", side=3, cex=2, adj=0)

#ammonium
# par(mai=c(1,1,1,7))
plot(cmop$a, cmop$dm, pch=16, xlab="ammonium", ylab="mean daily division rate", cex.lab=1.7, cex=1.5, xlim=c(0,14), cex.axis=1.7)
#res3=lm(cmop$dm~cmop$a)
res3 <- lmodel2(cmop$dm~cmop$a, data=cmop, nperm=0, range.x="relative", range.y="relative")
par(new=T)
plot(res3, "MA", ylab="", xlab="", main="", col="black", xlim=c(0,14), cex.axis=1.7)
text(11,8, substitute(paste("R"^{2}, "=0.076")), cex=2)
mtext("C", side=3, cex=2, adj=0)

#par(mai=c(1,1,1,7))
plot(cmop$par.cum, cmop$dm, pch=16, xlab="PAR", ylab="", cex.lab=1.7, cex=1.5, xlim=c(0,250), cex.axis=1.7)
#res4=lm(cmop$dm~cmop$par.cum)
res4 <- lmodel2(cmop$dm~cmop$par.cum, data=cmop, nperm=0, range.x="relative", range.y="relative")
par(new=T)
plot(res4, "MA", ylab="", xlab="", main="", col="black", xlim=c(0,250), cex.axis=1.7)
...
#total nitrogen 
# plot(cmop$tn, cmop$dm, pch=16, xlab="nitrogen", ylab="mean daily division rate", cex.lab=1.7, cex=1.5)
# #res1=lm(cmop$dm~cmop$n)
# res1 <- lmodel2(cmop$dm~cmop$tn, data=cmop.pro, nperm=0, range.x="relative", range.y="relative")
# par(new=T)
...
png(filename="~/Desktop/meso_corr_test.png", width=975, height=975, units="px", res=150)
#quartz("Quartz", width=6.5, height=6.5)
par(mfrow=c(2,2), mar=c(2,1.5,0.5,0.2)+2, pty="s", las=1, cex.axis=1)

#nitrate
#par(mai=c(1,1,1,7))
plot(cmop.meso$n.meso, cmop.meso$meso, pch=16, xlab="nitrate", ylab="M. major abundance", cex.lab=1, cex=1, xlim=c(0,14), cex.axis=1)
#mtext(text="nitrate", side=1, cex=1, outer=T, at=c(1,1))
#res1=lm(cmop$dm~cmop$n)
res1 <- lmodel2(cmop.meso$meso~cmop.meso$n.meso, data=cmop.meso, nperm=0, range.x="relative", range.y="relative")
par(new=T)
plot(res1, "MA", ylab="", xlab="", main="", col="black", xlim=c(0,14), cex.axis=1)
text(5.5,300, substitute(paste("R"^{2}, "=0.017")), cex=1)
mtext("A", side=3, cex=1.4, adj=0)

#phosphate
# par(mai=c(1,1,1,7))
plot(cmop.meso$ph.meso, cmop.meso$meso, pch=16, xlab="phosphate", ylab="", cex.lab=1, cex=1, xlim=c(0,1.3), cex.axis=1)
#res2=lm(cmop$dm~cmop$ph)
res2 <- lmodel2(cmop.meso$meso~cmop.meso$ph.meso, data=cmop.meso, nperm=0, range.x="relative", range.y="relative")
par(new=T)
plot(res2, "MA", ylab="", xlab="", main="", col="black", xlim=c(0,1.3), cex.axis=1)
text(0.55,300, substitute(paste("R"^{2}, "=0.068")), cex=1)
mtext("B", side=3, cex=1.4, adj=0)

#ammonium
# par(mai=c(1,1,1,7))
plot(cmop.meso$a.meso, cmop.meso$meso, pch=16, xlab="ammonium", ylab="M. major abundance", cex.lab=1, cex=1, xlim=c(0,14), cex.axis=1)
#res3=lm(cmop$dm~cmop$a)
res3 <- lmodel2(cmop.meso$meso~cmop.meso$a.meso, data=cmop.meso, nperm=0, range.x="relative", range.y="relative")
par(new=T)
plot(res3, "MA", ylab="", xlab="", main="", col="black", xlim=c(0,14), cex.axis=1)
text(11,300, substitute(paste("R"^{2}, "=0.081")), cex=1)
mtext("C", side=3, cex=1.4, adj=0)

#par(mai=c(1,1,1,7))
plot(cmop.meso$par.meso, cmop.meso$meso, pch=16, xlab="PAR", ylab="", cex.lab=1, cex=1, xlim=c(0,250), cex.axis=1)
#res4=lm(cmop$dm~cmop$par.cum)
res4 <- lmodel2(cmop.meso$meso~cmop.meso$par.meso, data=cmop.meso, nperm=0, range.x="relative", range.y="relative")
par(new=T)
plot(res4, "MA", ylab="", xlab="", main="", col="black", xlim=c(0,250), cex.axis=1)
text(70,300, substitute(paste("R"^{2}, "=0.196")), cex=1)
mtext("D", side=3, cex=1.4, adj=0)
...
png(filename="/Users/mariaham/CMOP/manuscript/fourth_draft_figures/pro_aux_correlation2_fig5.png", width=975, height=975, units="px", res=150)
...
#nitrate
#par(mai=c(1,1,1,7))
plotCI(cmop.pro$n, cmop.pro$production, uiw=cmop.pro$production.se, sfac=0, pch=16, xlab="nitrate", ylab="production", cex.lab=1, cex=1, ylim=c(0,1), xlim=c(0,14), cex.axis=1)
#res1=lm(cmop$dm~cmop$n)
res1 <- lmodel2(cmop.pro$production~cmop.pro$n, data=cmop.pro, nperm=0, range.x="relative", range.y="relative")
par(new=T)
plot(res1, "MA", ylab="", xlab="", main="", col="black", ylim=c(0,1), xlim=c(0,14), cex.axis=1)
text(3.5,0.9, substitute(paste("R"^{2}, "=0.752")), cex=1.3)
mtext("A", side=3, cex=1.5, adj=0)

#phosphate
# par(mai=c(1,1,1,7))
plotCI(cmop.pro$ph, cmop.pro$production, uiw=cmop.pro$production.se, sfac=0, pch=16, xlab="phosphate", ylab="", cex.lab=1, cex=1, ylim=c(0,1), xlim=c(0,1.3), cex.axis=1)
#res2=lm(cmop$dm~cmop$ph)
res2 <- lmodel2(cmop.pro$production~cmop.pro$ph, data=cmop.pro, nperm=0, range.x="relative", range.y="relative")
par(new=T)
plot(res2, "MA", ylab="", xlab="", main="", col="black", ylim=c(0,1), xlim=c(0,1.3), cex.axis=1)
text(0.35,0.9, substitute(paste("R"^{2}, "=0.127")), cex=1.3)
mtext("B", side=3, cex=1.5, adj=0)

#ammonium
# par(mai=c(1,1,1,7))
plotCI(cmop.pro$a, cmop.pro$production, uiw=cmop.pro$production.se, sfac=0, pch=16, xlab="ammonium", ylab="production", cex.lab=1, cex=1, ylim=c(0,1), xlim=c(0,14), cex.axis=1)
#res3=lm(cmop$dm~cmop$a)
res3 <- lmodel2(cmop.pro$production~cmop.pro$a, data=cmop.pro, nperm=0, range.x="relative", range.y="relative")
par(new=T)
plot(res3, "MA", ylab="", xlab="", main="", col="black", ylim=c(0,1), xlim=c(0,14), cex.axis=1)
text(3.5,0.9, substitute(paste("R"^{2}, "=0.056")), cex=1.3)
mtext("C", side=3, cex=1.5, adj=0)

#par(mai=c(1,1,1,7))
plotCI(cmop.pro$par.cum, cmop.pro$production, uiw=cmop.pro$production.se, sfac=0, pch=16, xlab="PAR", ylab="", cex.lab=1, cex=1, ylim=c(0,1), xlim=c(0,200), cex.axis=1)
#res4=lm(cmop$dm~cmop$par.cum)
res4 <- lmodel2(cmop.pro$production~cmop.pro$par.cum, data=cmop.pro, nperm=0, range.x="relative", range.y="relative")
par(new=T)
plot(res4, "MA", ylab="", xlab="", main="", col="black", ylim=c(0,1), xlim=c(0,200), cex.axis=1)
text(50,0.9, substitute(paste("R"^{2}, "=0.147")), cex=1.3)
mtext("D", side=3, cex=1.5, adj=0)
...
m.a <- (sum(m$div.ave, na.rm=T))/36
cc.a <- (sum(cc$div, na.rm=T))/14
...
# begin plot #
quartz("Quartz", width=10, height=5)
par(mfrow=c(1,2), mar=c(3,3.5,1,2)+1, pty="m")

plot(as.POSIXct(cc$time, origin="1970-01-01"), cc$div, ylim=c(0,0.05), sfrac=0, lwd=2, pch=16, cex=1,ylab=NA, xlab=NA, cex.axis=1, las=1, xaxt="n")

plotCI(m$time, m$div.ave, m$div.sd, col="darkgrey",add=T, sfrac=0, lwd=2, pch=16,cex=1, las=1)
axis.POSIXct(1, m$time2, at=seq(as.POSIXct("2014-09-22 20:00:00", origin="1970-01-01", tz='GMT'), max(m$time2, na.rm=T), by=60*60*4), format="%H:%M", cex.axis=1)

mtext(substitute(paste("division (h"^{-1},")")), side=2, line=3.2, cex=1)
legend("topleft", c("cell cycle", "model"), lty=c(1,1), lwd=c(3,3), col=c("black", "darkgrey"), cex=0.85)
mtext("A", side=3, cex=1.5, adj=0)


plot(both$m.new, both$cc.new, pch=16, cex=1, ylab=NA, xlab=NA, las=1, cex.axis=1)
mtext(substitute(paste("division (h"^{-1},")")), side=2, line=3.2, cex=1)
mtext(substitute(paste("division (h"^{-1},")")), side=1, line=2.5, cex=1)
...
text(0.018,0.04, substitute(paste("R"^{2}, "=0.597")), cex=1.3)
mtext("B", side=3, cex=1.5, adj=0)
