...
user <- '~/Documents/DATA/SeaFlow/CMOP/CMOP_git/manuscript/manuscript_V4/manuscript_Rcode/'
...
time.res <- cut(meso$Time,time.template)
daily.meso <-  tapply(meso$Meso, time.res, function(x) mean(x, na.rm=T))
...
data <- data.frame(cbind(time= time.template[-24],
    ...
    PROD=daily.prod.mean, DR=daily.dr.mean,PROD.sd = daily.prod.sd, DR.se = daily.dr.se, N=daily.abun.mean, CON = daily.con, meso=daily.meso))
data.dr <- data[id,]
...
P04.ph <- lmodel2(PH ~ P04, data,"relative", "relative", 99)
    ...
    DR.meso <- lmodel2(DR ~ meso, data,"relative", "relative", 99)
    prod.meso <- lmodel2(PROD ~ meso, data,"relative", "relative", 99)
...
png("FigureS1.png", width=114, height=114, pointsize=8, res=600, units="mm")
    ...
    map("worldHires", xlim=c(-124.5,-123.25), ylim=c(45.9,46.5), col="grey", fill=T, resolution=0)
    lat<-c(46.21)
    lon<-c(-123.91)
