...
nut <- read.csv(paste0(user, "/auxillary_data/Ribalet_nutrients2.csv"))[]
    ...
    time.template <- seq(min(nut$time), max(nut$time), by=60*60*24)[-1]
    time.res <- cut(nut$time,time.template)
...
plot(data$time, data$DR)
plot(data$time, data$N)
...
par(mfrow=c(3,1), mar=c(3,2,1,2), pty="m", cex=1.2, oma=c(1,3,1,3))
    ...
    lines(time.nut, daily.DIN, lwd=1.5)
    ...
    lines(time.nut, daily.PO4, lwd=1.5, col='darkgrey')
...
png("Figure4.png", width=114*1.5, height=114*1.5, pointsize=8, res=600, units="mm")

par(mfrow=c(3,1), mar=c(3,2,1,2), pty="m", cex=1.2, oma=c(1,3,1,3))

    plotCI(seq(min(rhodo$time), max(rhodo$time), by=60*60)[-1],h.abun.mean,uiw=h.abun.sd, sfrac=0, lwd=2, col='darkgrey', xlim=c(min(cc$time, na.rm=T),max(cc$time, na.rm=T)), ylim=c(0,30), pch=NA, xaxt='n', xlab=NA, ylab=NA, yaxt='n')
    lines(seq(min(rhodo$time), max(rhodo$time), by=60*60)[-1],h.abun.mean) 
...
par(mfrow=c(2,2), mar=c(3,2,1,2), pty="s", cex=1.2, oma=c(1,3,1,0))
    ...
    mtext("T. amphioxeia                      ", side=1, cex=1.2,  line=2.83, font=3)
