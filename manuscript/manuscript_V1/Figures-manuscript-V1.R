...
i <- min(df$h.time, na.rm=T)
f <- i + 60*60*24*3.5
d.rate.mean <- c(rep(NA, 23),rollsum(df$h.dr.mean, k=24))
d.rate.sd <- c(rep(NA, 23),rollsum(df$h.dr.sd, k=24))
id <- which(is.na(df$h2.conc.mean))
d.rate.mean[c(id,42:39)] <- NA
d.rate.sd[c(id,42:39)] <- NA
plotCI(df$h.time,d.rate.mean, uiw=d.rate.sd, sfrac=0, xaxt='n',xlab="", lwd=2, pch=16, ylab= "", cex.lab=1.5, col="darkgrey", las=1, ylim=c(0, 0.1), xlim=c(i,f), yaxt='n')
lines(df$h.time,d.rate.mean)
axis(2, las=1)
axis(1, at=seq(i, f, by=60*60*24), labels=seq(1,4,by=1))
rect(as.POSIXct("2013-09-10 23:51:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-09-11 06:13:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.15), border=NA)
rect(as.POSIXct("2013-09-11 11:52:00", origin="1970-01-01", tz='GMT'), 0.000000001, as.POSIXct("2013-09-11 17:41:00", origin="1970-01-01", tz='GMT'), 60.0, density=NULL, col=adjustcolor("black", alpha=0.15), border=NA)
...
mtext("A", side=3, cex=2, line=0, adj=0)
mtext(substitute(paste("Division rate (d"^{-1},')')), side=2, cex=1.2, outer=T, line=1)
...
d.rate.sd <- c(rep(NA, 23),rollsum(df$h.dr.sd, k=24))
id <- which(is.na(df$h2.conc.mean))
d.rate.mean[c(id,70:79)] <- NA
d.rate.sd[c(id,70:79)] <- NA
plotCI(df$h.time,d.rate.mean, uiw=d.rate.sd, sfrac=0, xaxt='n',xlab="", lwd=2, pch=16, ylab= "", cex.lab=1.5, col="darkgrey", las=1, ylim=c(0, 2.5), xlim=c(i,f), yaxt='n')
lines(df$h.time,d.rate.mean)
axis(2, las=1)
axis(1, at=seq(i, f, by=60*60*24), labels=seq(1,4,by=1)+6)
